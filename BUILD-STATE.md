...
## 2026-06-12 ~15:35 (interactive session) — Slack + Jira DEMO WORKSPACES SEEDED

- **Jira (incidentsherpa.atlassian.net): project PLAT created + 16 issues seeded** via the ACTIVE Composio connection (`scripts/seed_jira_history.py`, idempotent): 12 payments-service incidents Jan–May 2026 — 9 "Resolved by: dana-chen", 3 miguel-santos — the literal Senso 9-of-12 ownership source; INC-2417 mirror (PLAT-5) + INC-2289 mirror (PLAT-13) consistent with seed_senso.py verbatim; 14 issues transitioned Done; 2 deliberately open follow-ups for the Context Store to surface live: PLAT-14 (settlement-batch → read replica) and PLAT-16 (pool_used > 60 alert, "the alert that would have caught today's incident"). Labels + ADF descriptions verified by readback.
- **`JIRA_PROJECT_KEY` switched SCRUM → PLAT in .env** so the live JIRA_CREATE_ISSUE lands in the seeded project. NOTE: demo-scripts.md still says "PLAT-4417" — the live ticket will be PLAT-17+ (fresh site, counters can't be set); sync the on-screen copy at Phase 9 with whatever key the rehearsal run produces.
- **Slack ("Sherpa" workspace): #incidents seeded** (`scripts/seed_slack_history.py`, idempotent) — prior SHERPA INC-2417 update in the exact build_slack_update_text shape, PINNED; owner-confirm + postmortem chatter cross-linking PLAT-16; topic set; #eng-payments + #deploys created with 2 messages each. The 14:56 B7 connectivity-test message kept (real history). Satisfies the demo-scripts.md pre-staging line "#incidents has one prior SHERPA message".
- **B5-connectors HALF-UNBLOCKED:** the "real Jira project with seeded incident tickets" now exists. Remaining HUMAN step (~3 min): create an Atlassian API token (id.atlassian.com → Security → API tokens) and a GitHub PAT (`gh auth token` works), then provision the Airbyte Jira + GitHub sources (API or dashboard) against domain `incidentsherpa.atlassian.net`, project PLAT.
- **.env DRIFT WARNING:** a 15:18 rewrite (concurrent session) DROPPED the four Composio lines (key/user_id/channel/project). Restored 15:25 from docs/composio-b7-slack-jira-session.md. Loop firings: re-check these four lines before any Composio gate.
- **SECURITY — MUST FIX BEFORE PUBLIC FLIP:** the real `COMPOSIO_API_KEY` is committed in docs/composio-b7-slack-jira-session.md (commit d2fc007). The Devpost submission requires flipping this repo PUBLIC — rotate the Composio key (dashboard → Settings → API keys), update .env, and redact the doc BEFORE the flip. Key in git history = treat as burned at flip time.
