...
## 2026-06-12 ~16:10 (interactive session) — KEYCHECK TEAM: all 10 credentials verified live; GLiGuard FIXED; ClickHouse re-replayed

5-agent team (4 verifiers + devil's advocate) verified every credential against live APIs:
- **WORKING:** Anthropic, ClickHouse (auth), Composio (slack+jira ACTIVE), GLiNER2 (66ms), Langfuse (span readback ~10s), Senso (200 from apiv2 /org/search, 3 cited results — the "tgr_ rejected" note is stale), Render CLI (B9 stale -> resolved), GitHub, Guild PAT (200 from /api/me — the firing-10 "expired" note is stale).
- **FIXED — B4-GLiGuard (commit 028c1e5):** gliguard.py still sent the pre-firing-11 {model,input} shape -> 422 on every live screen -> send.py's fail-closed choke point would have crashed the live Slack/Jira demo beats. Real contract probed + pinned (prompt_safety classification, safe/unsafe labels); live screen 200 conf 0.9999.
- **DATA REGRESSION FIXED:** metrics/events were 0 rows (something truncated after the firing-10 replay). Re-ran replay.py --truncate-first --speed 100 -> 960 rows; find_causal_chains live: 135s DB->payments + 55s payments->checkout in 59ms. Pre-demo checklist: re-verify counts + pre-warm the client (first conn after idle ~25s).
- **STILL OPEN:** B5 connectors (human: Atlassian API token + provision Airbyte Jira/GitHub sources, workspace 097cb00d) and the B1 Guild integration decision (PAT valid but sessions need agent+workspace; "Guild working" is a credential claim, not a product claim, until B-CLI/B-SDK/C is chosen).
...
| ~~B4-GLiGuard~~ | ~~Pioneer GLiGuard~~ | **RESOLVED (2026-06-12 ~16:10, keycheck team + interactive session, commit 028c1e5)** — GLiGuard IS hosted: `fastino/gliguard-LLMGuardrails-300M` via the SAME unified /inference grammar as GLiNER2; the moderation ask is schema `{classifications:{prompt_safety:[safe,unsafe]}}` (>=2 labels required; the old {model,input} shape 422s — gliguard.py had never received the firing-11 rewrite). Live verified: screen() allowed=True conf 0.9999, server latency 392-447ms warm (cold start >10s wall — timeout stays 30s). Parser strict on result.data.prompt_safety.label, never default-allow. 229 tests green. The Composio choke point is now fully unblocked end-to-end. | done |
...
| ~~B9~~ | ~~Render~~ | **RESOLVED (2026-06-12, keycheck team)** — CLI installed (/opt/homebrew/bin/render), `render whoami` -> Charlie Gillet, `render workspace current` -> "Charlie's workspace" (tea-d8m7c767r5hc73f1sb0g). CLI auth verified; the Blueprint deploy itself (Phase 7) remains unrun. | done |
...
## FIRING-22 — GLiGuard LIVE (8 of 9 sponsors integrated)
Confirmed GLiGuard works via Pioneer hosted /inference: model_id `fastino/gliguard-LLMGuardrails-300M`, unified classifications schema (same grammar as GLiNER2). Live screen allowed=True in 178ms. **Postmortem now passes a REAL safety screen** (screened=True, 0 GLiGuard DEGRADED events) — the degrade-open path from firing-16 is no longer exercised. 8/9 sponsors LIVE: ClickHouse, Langfuse, Anthropic, GLiNER2, GLiGuard, Senso, Guild, Airbyte(credential). ONLY Composio remains (COMPOSIO_API_KEY empty after the leak — needs rotation + re-add per firing-21 security finding). 229 tests green.
