...
| Airbyte Agent Engine | THREE touchpoints (deepened post-convergence): (1) live Context Store semantic query at incident time — related Jira tickets/PRs in <500ms, latency badge on the timeline, span in the Langfuse waterfall; (2) MCP interface (shipped 06-05) as deliberate path or fallback; (3) GitHub + Jira connectors (Jira OAuth 2.0 confirmed 06-03) batch-pull 90-day history into ClickHouse for the ownership baseline. PagerDuty on-site stretch only. Pitch cites the 06-11 release (connector inspection + semantic-search precision). | Ownership suggestion is random without historical GitHub/Jira signal; the LIVE pre-indexed query (not batch load alone) is the literal "Context Store search, not live API fetch" judging hint. |
