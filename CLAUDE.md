...
- 2026-06-13 — For live UI/SSE verification (Phase 6/7) use a REAL uvicorn server, NOT fastapi TestClient: TestClient streaming (`/events`) + FastAPI BackgroundTasks deadlock in one process (firing-13). The product is fine under uvicorn; this is a test-harness limitation only.
- 2026-06-13 — Senso (and sponsor clients) are degradable on RUNTIME failure (500/timeout → DegradedError → DEGRADED event, pipeline continues — satisfies Phase 8) but a missing CREDENTIAL (NotConfiguredError) propagates by design: /health already reports it "blocked", and a degraded incident silently missing its runbook would be LESS honest than a loud failure. Do not "fix" this into a silent degrade.
...
- 2026-06-12 — Never write a wall-clock timestamp from estimation: run `date` at the moment of the event, or back-fill from `git log` commit times. Caught writing "2:55 PM" for an event that actually happened at 2:14 PM. The claim-integrity standard for latency numbers applies to times too. (Also: GLiGuard IS hosted — `fastino/gliguard-LLMGuardrails-300M` returned 200 with task `prompt_safety`, 426ms cold; supersedes the firing-11 note that it needs the local fallback.)
- 2026-06-12 — Composio SDK (composio 0.13.1, core 1.0.0-rc2) contradicts the authored `session.create()/session.tools.execute()/session.link()` guess on every point (same lesson as Pioneer firing-11): there is NO per-call session — `tools`, `connected_accounts`, `toolkits` hang directly off the `Composio` instance; sends are `client.tools.execute(slug, arguments, user_id=...)` (user_id routes to the connected account); OAuth is `connected_accounts.link(user_id, auth_config_id)` or `toolkits.authorize(user_id, toolkit)` → `ConnectionRequest(.redirect_url, .wait_for_connection())`; `initiate()` still exists but is the deprecated path. Also: the SDK creates `~/.composio` at IMPORT — pin `COMPOSIO_CACHE_DIR` to a writable dir or a read-only container home crashes the import. Confirmed by introspecting the installed SDK signatures (dir()/inspect.signature), not from docs. Rule reinforced: introspect the INSTALLED SDK the moment it's pip-installed.
