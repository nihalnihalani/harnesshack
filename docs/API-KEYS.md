...
8. **VERIFIED API shapes (June 12, 2:30 PM):** encoder models run on `POST https://api.pioneer.ai/inference` with `X-API-Key` (they are NOT in `/v1/models`, which lists 70 chat models behind the OpenAI-compatible router):
